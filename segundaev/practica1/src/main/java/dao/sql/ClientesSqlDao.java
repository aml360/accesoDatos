...
public class ClientesSqlDao implements IDao<Cliente> {
    ...
    public void guardar(Cliente t) throws ExcepcionCodigoYaExistente, SQLException {
        try (PreparedStatement count_cod_cliente = conn
                .prepareStatement("select count(*) from cliente where codigo_cliente=?;");
                PreparedStatement insert_into_cliente = conn
            ...
            if (rs.next()) {
                if (rs.getInt(1) != 0) {
                    ...
                    // catch try catch try catch try catch try
    ...
    /**
     * Saca el cliente al que está apuntando el ResultSet, y si existe y todos los
     * datos son correctos, devuelve un Optional de ese cliente. Si el ResultSet no
     * está apuntando a ningún Cliente, lanza una excepción. Si el ResultSet es null
     * o alguno de los datos en la base de datos es incorrecto, devuelve un Optional
     * vacío.
     * 
     * @param sqlQuery
     * @return Un Optional con un cliente válido, o un Optional vacío.
     * @throws SQLException Si no se ha llamado a next() antes de invocar a esta
     *                      función, o nosequé otras movidas le puedan disgustar a
     *                      esto.
     */
