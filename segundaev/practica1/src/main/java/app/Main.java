...
import dao.sql.ConexionJardineria;
import excepciones.ExcepcionCodigoYaExistente;
import excepciones.ExcepcionDatoNoValido;
...
public class Main {
	...
	public static void main(String[] args) {
		try (ConexionJardineria c = new ConexionJardineria()) {
			...
			System.out.println("Excepcion al cerrar conexion;");
			e.printStackTrace();
			System.out.println();
		}
	}
	...
	private static void acatar(int orden, ConexionJardineria c) {
		...
		case 0:
			return;
		case 1:
			pedirDatosYGuardarCliente(c);
			break;
		case 2:
			pedirCodigoYMostrarCliente(c);
			break;
		case 3:
			mostrarTodosLosClientes(c);
			break;
		case 4:
			pedirPalabraYBuscarClientes(c);
			break;
		case 5:
			pedirCodigoYEditarProducto(c);
			break;
		}
	}
	...
	private static void pedirCodigoYEditarProducto(ConexionJardineria c) {
		...
		Object[] params = { pedirString("codigo_producto", true), pedirString("nombre", true), pedirString("gama", true),
				pedirString("dimensiones", true), pedirString("proveedor", true), pedirString("descripcion", true),
				pedirEntero("cantidad_en_stock", true), pedirDouble("precio_venta"), pedirDouble("precio_proveedor") };
	...
	private static void pedirCodigoYMostrarCliente(ConexionJardineria c) {
		...
		c.clientes().uno(codigo).ifPresentOrElse(cli -> System.out.println(cli),
